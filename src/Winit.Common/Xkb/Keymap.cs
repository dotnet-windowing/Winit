...
public sealed class XkbKeymap : IDisposable
{
    ...
    internal static unsafe XkbKeymap? FromString(XkbContext context, string keymapText)
    {
        byte[] utf8 = System.Text.Encoding.UTF8.GetBytes(keymapText + '\0');
        fixed (byte* ptr = utf8)
        {
            nint keymap = PInvoke.xkb_keymap_new_from_string(
                context.Handle,
                (sbyte*)ptr,
                PInvoke.XkbKeymapFormatTextV1,
                PInvoke.XkbKeymapCompileNoFlags);
            return keymap == 0 ? null : new XkbKeymap(keymap);
        }
    }
